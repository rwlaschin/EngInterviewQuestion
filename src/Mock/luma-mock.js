...
LumaMock.prototype.getProviders = function(callback) {
	return callback(null, [
		{
			id: 1,
			name: "Doogie Howser", // mistake?
			firstname: "Doogie",
			lastname: "Howser" // mistake?
		},
		{
			id: 2,
			name: "Gregory House",
...
LumaMock.prototype.getPatient = function(id, facilityId, callback) {
	var dateOfBirth = moment(
		`${randomIntFromInterval(1, 12)}/${randomIntFromInterval(
			1,
			30
		)}/${randomIntFromInterval(1920, 2016)}`,
		"MM/DD/YYYY"
	);
	var patientObject = {};
...
LumaMock.prototype.getAppointments = function(start, end, callback) {
	...
	for (var i = 0; i < days * 2; i++) {
		var newAppt = {
			...
			date: moment(start) // bug fix, only from today, not start
				.add(i, "days")
				.hour(getRandomHour())
				.minute(0)
				.seconds(0)
...
LumaMock.prototype.updateAppointmentStatus = function(
	appointmentId,
	facilityId,
	status,
	date,
...
LumaMock.prototype.bookShadowAppointment = function(
	time,
	duration,
	provider,
	facility,
...
LumaMock.prototype.createAppointment = function(
	time,
	duration,
	type,
	provider,
